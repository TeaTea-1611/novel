...
import { loginSchema, newPasswordSchema, registerSchema } from "../../schemas";
import { env } from "../../env";
...
export class AuthResolver {
  ...
  async refreshToken(
    ...
    if (decoded?.sub) {
      const rt = await prisma.refreshToken.delete({
        where: { userId: decoded.sub, token },
      });

      if (rt) {
        const hasExpired = new Date(rt.expires) < new Date();

        if (!hasExpired) {
          const { accessToken, refreshToken } =
            await this.authService.createTokens(decoded.sub, prisma);
          this.authService.setRefreshTokenCookie(res, refreshToken);
          return accessToken;
        }
      }
