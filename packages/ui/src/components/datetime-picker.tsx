...
const timePickerInputBase =
  "inline tabular-nums h-fit border-none outline-none select-none content-box caret-transparent rounded-sm min-w-8 text-center focus:bg-foreground/20 focus-visible:ring-0 focus-visible:outline-none";
  ...
  (
    ...
    return (
      <div className="relative">
        <div
          className={cn(
            "flex items-center w-fit px-2 pb-2 pt-6 h-11 rounded-md shadow-sm transition-colors bg-background border border-input",
            className,
            "rounded-md gap-1 selection:bg-transparent selection:text-foreground",
          )}
                    ...
                    ? group.map((unit, j) => (
                        <React.Fragment key={unit}>
                          <input
                            className={cn(
                              timePickerInputBase,
                              "min-w-8 w-full rounded-md border border-input bg-background p-0 text-sm shadow-sm transition-colors",
                              {
                                "min-w-12": unit === "years",
        ...
        <label className="absolute block text-xs font-bold uppercase transition-all duration-300 top-1 left-2 text-muted-foreground">
          {label}
        </label>
      </div>
    );
